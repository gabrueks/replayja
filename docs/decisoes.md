...
- **Apagar branches Neon de teste** (precisam de confirmação do Gabriel): `teste-integracao-grupos` (`br-sparkling-bar-ac525n9y`) e `teste-painel-temporario` (`br-sparkling-base-acjja6u5`).
- Largura da marca do piloto voltou a **18%** (a 0011 tinha rebaixado para 12% no backfill).
- Correções pequenas pendentes: grade borrada da arena deslogada mostra marca "Arena Calabouço" de fixture; contador de lances esconde o clipe em processamento; expurgo sem as camadas de invalidação de URL assinada/relay; bloqueio de horário não atravessa meia-noite.
